...
static int vmwPutPixel(int x,int y,int color,GLubyte *texture) {
...
//		printf("%i : %i\n",(y*(64*4))+x,palette[color][0]);
		...
		if (texture_alpha && color==0) {
			texture[(y*(texture_x*4))+(x*4)+3]=0;
		}
		else {
			texture[(y*(texture_x*4))+(x*4)+3]=255;
		}
	}
	return 0;
}

static int vmwDrawHLine(int x,int y,int run,int color,GLubyte *target) {
...
static vmwPaintProHeader *vmwGetPaintProHeader(char *filename) {
...
static int vmwPutAlphaPixel(int x,int y,int color,GLubyte *texture) {
...
static int vmwDrawAlphaHLine(int x,int y,int run,int color,GLubyte *target) {
